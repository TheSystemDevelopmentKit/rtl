...
class rtl_iofile(verilog_iofile_obsoletes, rtl_iofile_common):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        **kwargs :
        ...
        try:
            ...
            self._ioformat = kwargs.get(
                "ioformat", "%d"
            )  # by default, the io values are decimal integer numbers
    ...
    def connector_datamap(self, **kwargs):
        ...
        if not self.rtl_connectors:
            ...
            for val in self.rtl_connectors:
                index += 1
                self._verilog_connector_datamap.update(
                    {"%s" % (val.name): index}
                )
    ...
    def set_control_data(self, **kwargs):
        ...
        **kwargs :
        ...
        assert isinstance(
            time, int
        ), "Argument 'time' should have the type 'int'"
        ...
        if self.DictData is None:
            ...
            if np.isscalar(init):
                self.DictData[0] = (
                    np.ones(len(self.rtl_connectors)) * init
                ).astype(int)
        ...
        # int -- change signal to the given value
            ...
            if time not in self.DictData:
                ...
                self.DictData[time] = [
                    None for _ in range(len(self.rtl_connectors))
                ]
    ...
    def Data(self):
        if not hasattr(self, "_Data"):
            ...
            if self.iotype == "event" and hasattr(self, "_DictData"):
                diff_array = np.array(
                    [
                        np.insert(signals, 0, time)
                        for (time, signals) in self.DictData.items()
                    ]
                )
