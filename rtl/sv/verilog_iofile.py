    ...
    def rtl_io_condition(self):
        ...
        if not hasattr(self, "_rtl_io_condition"):
            if self.parent.dir == "out":
                ...
                for connector in self.parent.rtl_connectors:
                    if first:
                        self._rtl_io_condition = "~$isunknown(%s)" % (
                            connector.name
                        )
                        first = False
                    else:
    ...
    def rtl_io(self):
        ...
        if self.parent.iotype == "sample":
            if self.parent.dir == "out":
                ...
                self._rtl_io += indent(
                    text="$fwrite(%s, " % (self.rtl_fptr), level=2
                )
            elif self.parent.dir == "in":
                ...
                self._rtl_io += indent(
                    text="%s = $fscanf(%s, " % (self.rtl_stat, self.rtl_fptr),
                    level=2,
                )
            for connector in self.parent.rtl_connectors:
                if first:
            ...
            format = format + '\\n",\n'
            self._rtl_io += (
                indent(text=format + iolines + "\n);", level=2)
                + indent(text="end", level=1)
                + indent(text="end", level=0)
            )
