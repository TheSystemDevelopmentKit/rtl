...
class verilog_module(module_common, thesdk):
    ...
    def ios(self):
        ...
        if not hasattr(self, "_ios"):
            ...
            def port_pattern(identifier):
                return rf"\s*(input|output|inout)?\s*(wire|reg)?\s*(\[\s*([0-9]+)\s*:\s*([0-9]+)\s*\])?\s*({identifier})"
            ...
            if not os.path.isfile(self.file):
                self.print_log(
                    type="F", msg="File does not exist: %s" % self.file
                )
    ...
    def parameters(self):
        ...
        if not hasattr(self, "_parameters"):
            startmatch = re.compile(
                r"module *(?=" + self.name + r"\s*\()\s*" + r".*.+$"
            )
            ...
            if os.path.isfile(self.file):
                with open(self.file) as infile:
                    ...
                    if parablock:
                        ...
                        func_list = [
                            lambda s, fil=x: re.sub(fil, "", s) for x in fils
                        ]
                        parablock = reduce(
                            lambda s, func: func(s), func_list, parablock
                        )
                ...
                with open(self.file) as infile:
                    ...
                    for line in wholefile:
                        ...
                        if modfind and headerstopmatch.match(line):
                            ...
                            modfind and headers and modulestopmatch.match(line)
                        ):
                            modfind = False
                            headers = False
                            # exclusive
            ...
            if self.parameters.Members:
                ...
                for name, val in self.parameters.Members.items():
                    if type(val) is not tuple:
                        self.print_log(
                            type="F",
                            msg="Parameter %s must be defined as {'<name>': ('<type>',value)}"
                            % (name),
                        )
                ...
                for ioname, io in self.ios.Members.items():
                    ...
                    if io.cls in ["input", "output", "inout"]:
                        if io.width == 1:
                            ...
                            self._definition = self._definition + (
                                "    %s [%s:%s] %s"
                                % (io.cls, io.ll, io.rl, io.name)
                            )
                    else:
                        self.print_log(
                            type="F",
                            msg="Assigning signal direction %s to verilog module IO."
                            % (io.cls),
                        )
                self._definition = self._definition + "\n)"
            self._definition = self._definition + ";"
    ...
    def export(self, **kwargs):
        ...
        if not os.path.isfile(self.file):
            ...
            self.print_log(
                msg="Forcing overwrite of verilog_module to %s." % (self.file)
            )
            with open(self.file, "w") as module_file:
                module_file.write(self.header + self.definition)
