...
class verilator(thesdk, metaclass=abc.ABCMeta):
    ...
    def verilator_rtlcmd(self):
        ...
        vlogmodulesstring = " ".join(
            ...
            + [
                ...
                for param in self.vlogmodulefiles
            ]
        )
        vhdlmodulesstring = " ".join(
            [
                self.rtlsimpath + "/" + str(param)
                for param in self.vhdlentityfiles
            ]
        )
        ...
        if vhdlmodulesstring != "":
            self.print_log(
                type="W",
                msg="Verilator does not support Verilog+VHDL cosimulation, ignoring additional VHDL files.",
            )
        ...
        if self.interactive_rtl:
            ...
            if os.path.isfile(dofile):
                dostring = ' -S "' + dofile + '"'
                self.print_log(
                    type="I", msg="Using interactive control file %s" % dofile
                )
            else:
                dostring = ""
    ...
    def verilator_controlfilepaths(self):
        controlfiledir = (
            "%s/interactive_control_files/verilator" % self.entitypath
        )
