...
class rtl(
    ...
    def rtl_timescale_num(self):
        ...
        if match:
            ...
            self.print_log(
                type="F",
                msg=f"Invalid rtl_timescale string: {val}. Allowed e.g. 1ps",
            )
        if text == "ms":
            number *= 1e-3
        elif text == "us":
            ...
            self.print_log(
                type="F",
                msg=f"Invalid rtl_timescale unit: {val}. Allowed ms,us,ns,ps,fs",
            )
        return number
    ...
    def rtl_timeprecision_num(self):
        ...
        if match:
            ...
            self.print_log(
                type="F",
                msg=f"Invalid rtl_timeprecision string: {val}. Allowed e.g. 1ps",
            )
        if text == "ms":
            number *= 1e-3
        elif text == "us":
            ...
            self.print_log(
                type="F",
                msg=f"Invalid rtl_timeprecision unit: {val}. Allowed ms,us,ns,ps,fs",
            )
        return number
    ...
    def rtl_timeunit(self):
        ...
        if not hasattr(self, "_rtl_timeunit"):
            self.print_log(
                type="O",
                msg="Use `rtl_timescale` and `rtl_timeprecision` instead",
            )
    ...
    def rtl_timeunit(self, val):
        self.print_log(
            type="O", msg="Use `rtl_timescale` and `rtl_timeprecision` instead"
        )
        ...
        if not hasattr(self, "_rtlsimpath"):
            ...
            try:
                if not os.path.exists(self._rtlsimpath):
                    self.print_log(
                        type="I", msg="Creating %s" % self._rtlsimpath
                    )
                    os.makedirs(self._rtlsimpath)
            except:
                self.print_log(
                    type="E", msg="Failed to create %s" % self.rtlsimpath
                )
        ...
        if os.path.exists(self.rtlsimpath):
            ...
            if not self.preserve_rtlfiles:
                try:
                    ...
                    self.print_log(
                        type="I", msg="Removing %s" % self.rtlsimpath
                    )
                except:
                    self.print_log(
                        type="W", msg="Could not remove %s" % self.rtlsimpath
                    )
    ...
    def delete_rtlworkpath(self):
        ...
        if os.path.exists(self.rtlworkpath):
            try:
                ...
                self.print_log(
                    type="W", msg="Could not remove %s" % self.rtlworkpath
                )
            ...
            try:
                ...
                if libfile == "":
                    ...
                    if libfile == "":
                        self.print_log(
                            type="W",
                            msg="Global TheSDK variable VLOGLIBFILE not set.",
                        )
                    else:
                        self.print_log(
                            type="I", msg="Using VLOGLIBFILE: %s" % libfile
                        )
                        try:
                            ...
                            self.print_log(
                                type="F",
                                msg="Could not read verilog module files from VLOGLIBFILE:\n\t%s"
                                % e,
                            )
            except:
            ...
            try:
                ...
                if libfile == "":
                    ...
                    if libfile == "":
                        self.print_log(
                            type="W",
                            msg="Global TheSDK variable VHDLLIBFILE not set.",
                        )
                    else:
                        self.print_log(
                            type="I", msg="Using VHDLLIBFILE: %s" % libfile
                        )
                        try:
                            with open(libfile, "r") as fd:
                                modulefiles = [
                            ...
                            self.print_log(
                                type="F",
                                msg="Could not read verilog module files from VHDLLIBFILE:\n\t%s"
                                % e,
                            )
            except:
    ...
    def simulator_controlfile(self):
        ...
        if self.model == "icarus":
            (controlfiledir, controlfile, generatedcontrolfile) = (
                self.icarus_controlfilepaths
            )
        elif self.model == "sv":
            (controlfiledir, controlfile, generatedcontrolfile) = (
                self.questasim_controlfilepaths
            )
        elif self.model == "vhdl":
            (controlfiledir, controlfile, generatedcontrolfile) = (
                self.questasim_controlfilepaths
        ...
        if not hasattr(self, "_simulator_controlfile"):
            ...
            if self.simulator_control_contents != "":
                ...
                if os.path.isfile(controlfile):
                    self.print_log(
                        type="W",
                        msg="Simulator control file %s ignored and simulator_control_contents used instead."
                        % controlfile,
                    )
                # Write simulator_control_contents to a temporary file
                self.print_log(
                    type="I",
                    msg="Writing simulator_control_contents to file %s"
    ...
    def interactive_controlfile(self):
        ...
        if self.model == "icarus":
            (dofiledir, dofile, obsoletedofile, generateddofile) = (
                self.icarus_dofilepaths
            )
        elif self.model == "sv":
            (dofiledir, dofile, obsoletedofile, generateddofile) = (
                self.questasim_dofilepaths
            )
        elif self.model == "vhdl":
            (dofiledir, dofile, obsoletedofile, generateddofile) = (
                self.questasim_dofilepaths
        ...
        if not hasattr(self, "_interactive_controlfile"):
            ...
            if os.path.exists(obsoletedofile):
                self.print_log(
                    type="O",
                    msg="Found obsoleted do-file in %s" % obsoletedofile,
                )
                self.print_log(type="O", msg="To fix the obsolete warning:")
                self.print_log(
                    type="O",
                    msg="Move the obsoleted file %s to the default path %s"
                    % (obsoletedofile, dofile),
                )
                self.print_log(
                    type="O",
                    msg="Or, set a custom do-file path to self.interactive_controlfile.",
                )
                self.print_log(
                    type="O",
                    msg="Or, define the do-file contents in self.interactive_control_contents in your testbench.",
                )
                self.print_log(
                    type="O", msg="Using the obsoleted file for now."
                )
            ...
            if self.interactive_control_contents != "":
                ...
                if os.path.isfile(dofile):
                    self.print_log(
                        type="W",
                        msg="Interactive control file %s ignored and interactive_control_contents used instead."
                        % dofile,
                    )
                # Write interactive_control_contents to a temporary file
                self.print_log(
                    type="I",
                    msg="Writing interactive_control_contents to file %s"
                    % generateddofile,
                )
    ...
    def connect_inputs(self):
        ...
        for ioname, io in self.IOS.Members.items():
            if ioname in self.iofile_bundle.Members:
                ...
                if (
                    not isinstance(self.IOS.Members[val.name].Data, rtl_iofile)
                    and val.dir == "in"
                ):
                    # Data must be properly shaped
                    self.iofile_bundle.Members[ioname].Data = self.IOS.Members[
                        ioname
                    ].Data
        ...
        for ioname, val in self.iofile_bundle.Members.items():
            if val.ionames:
                for assocname in val.ionames:
                    if val.dir == "out":
                        if (val.datatype == "sint") or (
                            val.datatype == "scomplex"
                        ):
                            self.tb.connectors.Members[assocname].type = (
                                "signed"
                            )
                    self.tb.connectors.Members[assocname].ioformat = (
                        val.ioformat
                    )
            else:
                self.print_log(
                    type="F",
                    msg="List of associated ionames not defined for IO %s\n. Provide it as list of strings"
                    % (ioname),
                )
    ...
    def copy_or_relink(self, **kwargs):
        ...
        if os.path.islink(src):
            if not os.path.islink(dst):
                os.symlink(
                    os.path.join(os.path.dirname(src), os.readlink(src)), dst
                )
            else:
                os.remove(dst)
                os.symlink(
    ...
    def copy_rtl_sources(self):
        ...
        self.rtlfiles += (
            ...
            + self.vlogmodulefiles
            + self.vhdllibfileentities
            + self.vhdlentityfiles
        )
        ...
        for modfile in self.rtlfiles:
            ...
            if os.path.isfile(dstfile):
                self.print_log(
                    type="I",
                    msg="Using externally generated source: %s" % modfile,
                )
            else:
                self.print_log(
                    type="I", msg="Copying %s to %s" % (srcfile, dstfile)
                )
        ...
        output = subprocess.check_output(
            "sync %s" % self.rtlsimpath, shell=True
        )
    ...
    def execute_rtl_sim(self):
        ...
        try:
            if self.workdir:
                self.print_log(
                    type="I", msg=f"Executing in directory {self.workdir}"
                )
                execpath = self.workdir
            else:
                self.print_log(
                    type="I", msg=f"Executing in directory {self.rtlsimpath}"
                )
    ...
    def connect_outputs(self):
        ...
        for name, val in self.iofile_bundle.Members.items():
            if val.dir == "out":
                self.IOS.Members[name].Data = self.iofile_bundle.Members[
                    name
                ].Data
