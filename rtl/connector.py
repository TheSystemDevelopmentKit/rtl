...
class rtl_connector(thesdk):
    ...
    def __init__(self, **kwargs):
        ...
        self._connect = kwargs.get(
            "connect", None
        )  # Can be another connector, would be recursive
...
class rtl_connector_bundle(Bundle):
    ...
    def new(self, **kwargs):
        ...
        connect = kwargs.get(
            "connect", None
        )  # Can't be verilog connector by default. Would be recursive
        self.Members[name] = rtl_connector(
            lang=self.lang,
            name=name,
            cls=cls,
            type=type,
    ...
    def rtl_inits(self, **kwargs):
        ...
        for name, val in self.Members.items():
            if re.match(match, name) and (
                val.init is not None and val.init != ""
            ):
                # inits=inits+'%s = %s;\n' %(val.name,val.init)
