...
class verilator_module(thesdk):
    ...
    def ios(self):
        ...
        if not hasattr(self,'_ios'):
            ...
            self._ios=verilator_connector_bundle()
    ...
    def io_signals(self):
        ...
        if not hasattr(self,'_io_signals'):
            self._io_signals=verilator_connector_bundle()
    ...
    def export(self,**kwargs):
        ...
        if not os.path.isfile(self.file):
            self.print_log(msg='Exporting verilator_module to %s.' %(self.file))
            ...
            self.print_log(msg='Forcing overwrite of verilator_module to %s.' %(self.file))
            with open(self.file, "w") as module_file:
                module_file.write(self.definition)
