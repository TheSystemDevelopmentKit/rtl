...
class vhdl_connector(thesdk):
            ...
            if type(self.parent.width) == str:
                if self._type != "signed":
                    self.print_log(
                        type="I",
                        msg=(
                            "Setting type of '%s' to 'None' due to parametrized width."
                            % (self.parent.name)
                        ),
                    )
                    self._type = None
    ...
    def type(self, value):
        self.print_log(
            type="I",
            msg="Setting type of '%s' to '%s' " % (self.parent.name, value),
        )
        self._type = value
    ...
    def vhdl_signal_type(self):
        ...
        if not hasattr(self, "_vhdl_signal_type"):
            ...
            if type(self.parent.width) == str:
                ...
                self.parent.width == 1 and self._vhdl_signal_type != "std_logic"
            ):
                self.print_log(
                    type="I",
                    msg=(
                        "Setting vhdl_signal_type of '%s' to type 'std_logic' for VHDL simulations."
                        % (self.parent.name)
                    ),
                )
                self._vhdl_signal_type = "std_logic"
                ...
                self.parent.width > 1
                and self._vhdl_signal_type != "std_logic_vector"
            ):
                self.print_log(
                    type="I",
                    msg=(
                        "Setting vhdl_signal_type of '%s' to 'std_logic_vector' for VHDL simulations."
                        % (self.parent.name)
                    ),
                )
                self._vhdl_signal_type = "std_logic_vector"
