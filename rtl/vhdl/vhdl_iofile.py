...
class vhdl_iofile(rtl_iofile_common):
    ...
    def rtl_timescale(self):
        ...
        """
        if not hasattr(self, "_rtl_timescale") and hasattr(
            self.parent, "rtl_timescale"
        ):
            number = (
                int(re.search("^\d+", self.parent.rtl_timescale).group(0)) * 1.0
            )
    ...
    def rtl_statdef(self):
        ...
        if self.parent.iotype == "sample":
            ...
            for connector in self.parent.rtl_connectors:
                self._rtl_statdef += (
                    "variable status_%s : Boolean := False;\n"
                    % (connector.name)
                )
            ...
            self._rtl_statdef += (
                "--Time is presented as integers of time units\n"
            )
    ...
    def rtl_io_condition(self):
        ...
        if not hasattr(self, "_rtl_io_condition"):
            if self.parent.dir == "out":
                ...
                for connector in self.parent.rtl_connectors:
                    if first:
                        self._rtl_io_condition = "not is_x(%s)" % (
                            connector.name
                        )
                        first = False
                    else:
                        self._rtl_io_condition = "%s \n and not is_x(%s)" % (
                            self._rtl_io_condition,
                            connector.name,
                        )
    ...
    def rtl_io(self):
        ...
        for connector in self.parent.rtl_connectors:
            if connector.width == 1:
                if connector.ioformat == "%d":
                    self._rtl_io += indent(
                        text="variable v_%s : integer;" % (connector.name),
                        level=1,
                    )
                elif connector.ioformat == "%s":
                    self._rtl_io += indent(
                        text="variable v_%s : std_logic;" % (connector.name),
                        level=1,
                    )
            else:
                if connector.ioformat == "%d":
                    ...
                    self._rtl_io += indent(
                        text="variable v_%s : std_logic_vector( %s downto %s);"
                        % (connector.name, connector.ll, connector.rl),
                        level=1,
                    )
        ...
        if self.parent.iotype == "sample":
            if self.parent.dir == "out":
                self._rtl_io += "begin\n"
                self._rtl_io += indent(
                    text="while not thesdk_file_io_completed loop\n", level=1
                )
                self._rtl_io += indent(
                ...
                for connector in self.parent.rtl_connectors:
                    ...
                    if connector.width == 1:
                        if connector.ioformat == "%d":
                            self._rtl_io += indent(
                                text='v_%s := to_integer(unsigned\'("0" & %s));\n'
                                % (connector.name, connector.name),
                                level=4,
                            )
                        elif connector.ioformat == "%s":
                            self._rtl_io += "v_%s := to_string(%s)" % (
                                connector.name,
                                connector.name,
                            )
                        else:
                            self.print_log(
                                type="F",
                                msg="Connector format %s not supported"
                                % (connector.ioformat),
                            )
                    else:
                        if connector.ioformat == "%d":
                            if connector.type == "signed":
                                self._rtl_io += indent(
                                    text="v_%s := to_integer(signed(%s));\n"
                                    % (connector.name, connector.name),
                                    level=4,
                                )
                            else:
                                self._rtl_io += indent(
                                    text="v_%s := to_integer(unsigned(%s));\n"
                                    % (connector.name, connector.name),
                                    level=4,
                                )
                        elif connector.ioformat == "%s":
                    ...
                    if first:
                        self._rtl_io += indent(
                            text="write(line_%s,v_%s);"
                            % (self.rtl_fptr, connector.name),
                            level=3,
                        )
                        first = False
                    else:
                        self._rtl_io += indent(
                            text="write(line_%s, HT);" % (self.rtl_fptr),
                            level=4,
                        )
                        self._rtl_io += indent(
                            text="write(line_%s,v_%s);"
                ...
                self._rtl_io += indent(
                    text="writeline(%s,line_%s);\n"
                    % (self.rtl_fptr, self.rtl_fptr),
                    level=3,
                )
                ...
                for connector in self.parent.rtl_connectors:
                    self._rtl_io += indent(
                        text="read(line_%s,v_%s,status_%s);\n"
                        % (self.rtl_fptr, connector.name, connector.name),
                        level=4,
                    )
                    # verilog-like formatting
                    if connector.ioformat == "%d":
                        ...
                        if connector.width == 1:
                            self._rtl_io += indent(
                                text=(
                                    "%s <= std_logic(to_unsigned(v_%s,1)(0));\n"
                                    % (connector.name, connector.name)
                                ),
                                level=4,
                            )
                            ...
                            self._rtl_io += indent(
                                text=(
                                    "%s <= std_logic_vector(to_signed(v_%s,%s));\n"
                                    % (
                                        connector.name,
                                        connector.name,
                                        connector.width,
                ...
                self._rtl_io += indent(text="end loop;", level=1)
                self._rtl_io += indent(
                    text="done_%s <= True;" % (self.rtl_fptr), level=1
                )
                self._rtl_io += indent(text="%s" % (self.rtl_fclose), level=1)
        ...
        # Control files are handled differently
            ...
            if self.parent.dir == "out":
                self.print_log(
                    type="F",
                    msg="Output writing for control files not supported",
                )
            elif self.parent.dir == "in":
                self._rtl_io += "begin\n"
                self._rtl_io += indent(
                ...
                self._rtl_io += indent(
                    text="read(line_%s,%s,status_%s);\n"
                    % (self.rtl_fptr, self.rtl_ctstamp, self.rtl_ctstamp),
                    level=3,
                )
                for connector in self.parent.rtl_connectors:
                    self._rtl_io += indent(
