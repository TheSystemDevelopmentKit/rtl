...
class vhdl_testbench(testbench_common):
    ...
    def parameter_definitions(self):
        ...
        for name, val in self.content_parameters.items():
            if type(val) is not tuple:
                self.print_log(
                    type="F",
                    msg="Parameter %s definition must be given as { 'name' : (type,value) }"
                    % (name),
                )
            definitions += (
                "constant " + name + " : " + val[0] + ":=" + val[1] + ";\n"
            )
        return definitions
    ...
    def content_parameters(self):
        ...
        if not hasattr(self, "_content_parameters"):
            ...
            self._content_parameters = {
                "c_Ts": (
                    "time",
                    f"1.0/(g_Rs*{self.rtl_timescale_num:.1e})*1.0 {unit}",
                )
            }
    ...
    def end_condition(self):
        ...
        if not hasattr(self, "_end_condition"):
            if hasattr(self.parent, "end_condition"):
                ...
                self._end_condition = (
                    "thesdk_simulation_completed <= thesdk_file_io_completed;"
                )
    ...
    def define_testbench(self):
        ...
        if "clock" not in self.dut_instance.ios.Members:
            self.connectors.Members["clock"] = rtl_connector(
        ...
        if "reset" not in self.dut_instance.ios.Members:
            self.connectors.Members["reset"] = rtl_connector(
                lang=self.parent.lang, name="reset", cls="reg", init="'0'"
            )
        ...
        for ioname, val in self.parent.IOS.Members.items():
            if val.iotype != "file":
                ...
                if val.dir == "in":
                    ...
                    self.parent.iofile_bundle.Members[ioname].Data = (
                        self.parent.IOS.Members[ioname].Data
                    )
            elif (
                val.iotype == "file"
            ):  # If the type is file, the Data is a bundle
                for bname, bval in val.Data.Members.items():
                    if val.dir == "in":
                        # Adoption transfers parenthood of the files to this instance
                        self.IOS.Members[ioname].Data.Members[bname].adopt(
                            parent=self
                        )
                    for connector in bval.rtl_connectors:
                        ...
                        # Connect them to DUT
                        try:
                            self.dut.ios.Members[connector.name].connect = (
                                connector
                            )
                        except:
                            pass
