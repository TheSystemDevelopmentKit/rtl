...
class ghdl(thesdk):
    ...
    def ghdl_rtlcmd(self):
        ...
        vhdlmodulesstring = " ".join(
            ...
            + [
                self.rtlsimpath + "/" + str(param)
                for param in self.extract_vhdlfiles()
            ]
        )
        ...
        if vlogmodulesstring != "":
            self.print_log(
                type="W",
                msg="GHDL does not support Verilog+VHDL cosimulation, ignoring additional Verilog files.",
            )
        # We need to compile VHDL source and testbench anyway
        vhdlcompcmd = (
            "ghdl -a -Wall -Wno-unused --std=08 --workdir="
            + self.rtlworkpath
            + " "
            + vhdlmodulesstring
        )
        vhdlanalysiscmd = (
            "ghdl -e --std=08 --workdir="
            + self.rtlworkpath
            + " "
            + "tb_"
            + self.name
        ...
        if os.path.isfile(controlfile):
            ...
            self.print_log(
                type="I", msg="Using simulator control file %s" % controlfile
            )
        else:
            controlstring = ""
        ...
        if os.path.isfile(interactive_controlfile):
            ...
            self.print_log(
                type="I",
                msg="Using interactive control file %s"
